...
local function chaseCell(sim, x, y, agent, hunters)
    hunters = hunters or {}
    local closestGuard = simquery.findClosestUnit(
            sim:getNPC():getUnits(), x, y, function(guard)
                return (guard:getBrain() and not guard:isKO() and
                               not guard:getTraits().noInterestDistraction and
                               guard:getBrain():getSituation().ClassType ~= simdefs.SITUATION_COMBAT and
                               guard:getBrain():getSituation().ClassType ~= simdefs.SITUATION_FLEE and
                               not hunters[guard:getID()])
            end)
...
local function huntCell(sim, x, y, agent, hunters)
    ...
    local closestGuard = simquery.findClosestUnit(
            sim:getNPC():getUnits(), x, y, function(guard)
                return (guard:getBrain() and not guard:isKO() and
                               not guard:getTraits().noInterestDistraction and
                               not guard:getTraits().pacifist and
                               guard:getBrain():getSituation().ClassType ~= simdefs.SITUATION_COMBAT and
                               guard:getBrain():getSituation().ClassType ~= simdefs.SITUATION_FLEE and
                               not hunters[guard:getID()])
            end)
...
local function isPlayerAgent(unit)
    return unit and unit:isPC() and simquery.isAgent(unit) and unit:getLocation() and
...
local function royaleFlushApplyPenalties(sim, unit, penalties, hunters)
    ...
    if uiTxt then
        ...
        sim:dispatchEvent(
                ...
                {txt = uiTxt, x = x, y = y, unit = unit, color = cdefs.COLOR_PLAYER_WARNING})
    end
...
local function royaleFlushUnitStartTurn(sim, unit, hunters)
    ...
    elseif simRoom.backstabState == 0 then
        royaleFlushApplyPenalties(
                sim, unit, sim:getParams().difficultyOptions.backstab_redPenalties, hunters)
    elseif simRoom.backstabState == 1 then
        royaleFlushApplyPenalties(
                sim, unit, sim:getParams().difficultyOptions.backstab_yellowPenalties, hunters)
    end
end
...
local function royaleFlushDoor(sim, unit, doorCell1, doorCell2)
    ...
    if chase then
        ...
        sim:dispatchEvent(
                simdefs.EV_UNIT_FLOAT_TXT,
                {txt = uiTxt, x = doorCell.x, y = doorCell.y, color = cdefs.COLOR_CORP_WARNING})
    end
end
local function royaleFlushSafe(sim, unit, safeUnit)
    ...
    if chase then
        ...
        sim:dispatchEvent(
                simdefs.EV_UNIT_FLOAT_TXT,
                {txt = uiTxt, x = sx, y = sy, color = cdefs.COLOR_CORP_WARNING})
    end
end
local function royaleFlushAttacked(sim, attackedUnit, x, y, hunters)
    ...
    if hunter then
        ...
        sim:dispatchEvent(
                simdefs.EV_UNIT_FLOAT_TXT,
                {txt = uiTxt, x = x, y = y, color = cdefs.COLOR_CORP_WARNING})
    end
end
...
local function royaleFlushZoneDesc(penalties)
    ...
    desc = triggeredPenaltiesDesc(
            penalties.doorAlarm == "n", penalties.safeAlarm == "n", penalties.attackAlarm == "n",
            false)
    if desc then
        table.insert(descs, desc)
    end
    desc = triggeredPenaltiesDesc(
            penalties.doorAlarm == "a", penalties.safeAlarm == "a", penalties.attackAlarm == "a",
            true)
    if desc then
        table.insert(descs, desc)
    end
...
local function pickIcon(params)
    ...
    local hasNoCustom = (not yellowPenalties.disarm and not yellowPenalties.locate and
                                not yellowPenalties.attackAlarm)

    -- Options enabled by the standard preset.
    local hasStandard = (yellowPenalties.doorAlarm and yellowPenalties.safeAlarm and
                                redPenalties.doorAlarm == "a" and redPenalties.safeAlarm == "a" and
                                redPenalties.attackAlarm == "a" and redPenalties.locate and
                                redPenalties.locateAlarm == "a")
    local hasExactlyStandard = (yellowPenalties.doorAlarm == "n" and yellowPenalties.safeAlarm ==
                                       "n")
    -- Options disabled by the standard preset, but enabled by the classic preset.
    local hasNoClassic = (yellowPenalties.mp == 0 and redPenalties.mp == 0 and
                                 not redPenalties.disarm)

    -- Options enabled by the classic preset.
    local hasClassic = (yellowPenalties.mp >= 2 and yellowPenalties.noSprint and redPenalties.mp >=
                               4 and redPenalties.noSprint and redPenalties.disarm and
                               redPenalties.locate and redPenalties.locateAlarm == "a")
    local hasExactlyClassic = (yellowPenalties.mp == 2 and redPenalties.mp == 4)
    -- Options disabled by the classic preset, but enabled by the standard preset.
    local hasNoStandard = (not yellowPenalties.doorAlarm and not yellowPenalties.safeAlarm and
                                  not redPenalties.doorAlarm and not redPenalties.safeAlarm and
                                  not redPenalties.attackAlarm)
...
local npc_abilities = {
    backstab_royaleFlush = util.extend(createDaemon(STRINGS.BACKSTAB.DAEMONS.ROYALE_FLUSH)) {
        ...
        onTrigger = function(self, sim, evType, evData, userUnit)
            if evType == simdefs.TRG_START_TURN and sim:getCurrentPlayer():isPC() then
                ...
                if self._reverseZones and sim:backstab_isBackstabComplete() then
                    ...
                    sim:dispatchEvent(
                            simdefs.EV_SHOW_REVERSE_DAEMON, {
                                name = daemonStrings.REVERSE_NAME,
                                icon = self.icon,
                                txt = daemonStrings.REVERSE_DESC,
                            })
                end
                ...
                if sim:backstab_advanceZones() then
                    ...
                    local daemonStrings = STRINGS.BACKSTAB.DAEMONS.ROYALE_FLUSH
                    local txt = nextTurns and util.sformat(daemonStrings.WARNING, nextTurns) or
                                        daemonStrings.FINISHED_WARNING
                    sim:dispatchEvent(
                            simdefs.EV_BACKSTAB_SHOW_WARNING, {
                                txt = txt,
                                color = cdefs.COLOR_CORP_WARNING,
                                icon = self.icon,
                                sound = "SpySociety/HUD/mainframe/firewall_increase",
                            })
            ...
            elseif evType == simdefs.TRG_UNIT_USEDOOR and sim:getCurrentPlayer():isPC() and
                    evData.unit and evData.unit:isPC() then
                royaleFlushDoor(sim, evData.unit, evData.cell, evData.tocell)
            elseif evType == simdefs.TRG_SAFE_LOOTED and evData.targetUnit:getTraits().safeUnit and
                    evData.unit and evData.unit:isPC() then
                royaleFlushSafe(sim, evData.unit, evData.targetUnit)
            elseif evType == simdefs.TRG_UNIT_KO and evData.ticks and evData.unit and
                    evData.unit:isNPC() then
            ...
            elseif evType == simdefs.TRG_UNIT_KILLED and evData.unit and
                    evData.unit:getTraits().isGuard and evData.corpse then
                if self._attackQueue then
        ...
        onTooltip = function(self, hud, sim, player)
            ...
            section:addAbility(
                    strings.BLUE_DESC_TITLE, blueDesc,
                    "gui/icons/action_icons/Action_icon_Small/actionicon_noentry.png")

            local title
            title = strings.YELLOW_DESC_TITLE
            for _, yellowDesc in ipairs(
                    royaleFlushZoneDesc(
                            difficultyOptions.backstab_yellowPenalties)) do
                section:addAbility(
                        title, yellowDesc,
                        "gui/icons/action_icons/Action_icon_Small/actionicon_noentry.png")
