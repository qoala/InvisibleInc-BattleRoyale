...
function createBackstabGridProp(game, simCore, params)
    ...
    grid:initRectGrid(
            boardWidth * 2, boardHeight * 2, cdefs.BOARD_TILE_SIZE / 2, cdefs.BOARD_TILE_SIZE / 2)
...
function boardrig:init(layers, levelData, game, ...)
    ...
    if simCore:getParams().difficultyOptions.backstab_enabled then
        local overlayGrid, _, overlayAnim = createBackstabGridProp(
                game, simCore, cdefs.BACKSTAB.OVERLAYTILES)
