...
function simengine:init(...)
    ...
    if startTurn and self._rooms then
        ...
        for _, room in ipairs(self._rooms) do
            ...
            elseif room.backstab_alwaysRed or (room.tags and room.tags.backstab_alwaysRed) or
                    room.backstab_exitDistance >= 1000 then
        ...
        for i, room in ipairs(rooms) do
            ...
            simlog(
                    "LOG_BACKSTAB", "room=%s toExit=%s zone=%s", room.roomIndex,
                    room.backstab_exitDistance, backstabZone)
...
function simengine:backstab_turnsUntilNextZone(turnOffset)
    if self._backstab_nextZoneTurn and
            (not self._backstab_nextZone or self._backstab_nextZone <= self._backstab_maxZone) then
